...
const std::vector<std::unique_ptr<Graph::Vertex>> &Graph::get_vertices() const
{ return vertices; }
...
void Graph::remove_edge(int u, int v)
{
    if (u >= size())
    {
        std::cout << "tail node doesn't exist in graph, not removing" << std::endl;
        return;
    }
    auto tail = vertices[u].get();
    auto head = std::find(tail->adj_edges.begin(), tail->adj_edges.end(), std::make_pair(v,1));
    if (head == tail->adj_edges.end())
    {
        std::cout << "edge doesn't exist for this tail node" << std::endl;
        return;
    }
    else
    {
       tail->adj_edges.erase(head);
    }
}
